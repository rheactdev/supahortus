  ...
  return (
    <div className="w-full h-full flex flex-col">
      <div className="flex justify-between items-end mb-8">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Manage Gardens</h1>
          <p className="text-base-content/60 mt-1">
            Select a garden to manage settings and members.
          </p>
        </div>
        <Link href="/admin/gardens/create-new" className="btn btn-primary btn-sm">
          + New Garden
        </Link>
      </div>
      ...
      {gardens.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {gardens.map((garden) => (
            <Link
              key={garden.id}
              href={`/admin/gardens/${garden.slug}`}
